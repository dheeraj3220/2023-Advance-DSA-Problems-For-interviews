...
public:
    ...
    int numEnclaves(vector<vector<int>>& board) {
        ...
        for(int i=0;i<n;i++){
            ...
            if(board[m-1][i]==1 && !visited[m-1][i]) dfs(board,visited,m-1,i);    // boundary Taversal
        }
        for(int i=0;i<m;i++){
            if(board[i][0]==1 && !visited[i][0]) dfs(board,visited,i,0);
            if(board[i][n-1]==1 && !visited[i][n-1]) dfs(board,visited,i,n-1);     //boundary Trversal
        }
        for(int i=0;i<m;i++){
            for(int j=0;j<n;j++){
                if(board[i][j]==1 && !visited[i][j]) count++;                     //counting Elements
            }
        }
        return count;
